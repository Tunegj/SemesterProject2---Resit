...
/**
 * Generates the HTML for the "Create Pet Listing" page, which includes a form for adding a new pet to the adoption list.
 * The form collects various details about the pet, such as name, species, breed, age, gender, size, color, adoption status, location, image URL, image alt text, and description.
 * @returns A string containing the HTML for the "Create Pet Listing" page.
 */
...
/**
 * Initializes the "Create Pet Listing" page by setting up event listeners for the form submission and the age unknown checkbox.
 * This function should be called once when the "Create Pet Listing" page is loaded to ensure that the form behaves correctly and provides feedback to the user.
 */
export function initCreatePetPage(): void {
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      window.setTimeout(() => {
        window.location.hash = `#/listing?id=${createdPet.id}`;
      }, 800);
    } catch (error) {
      const message =
        error instanceof Error
          ? error.message
          : "An unknown error occurred while creating the pet listing.";
